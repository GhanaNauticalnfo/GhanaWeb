 ...
 */
    ...
    .directive('mapMousePosition', ['MapService', function (MapService) {
        return {
            ...
            replace: true,
            require: '^olMap',
            template: "<div class='map-mouse-position'>{{(filterPos) ? (currentPos | lonlat:{ decimals : 2, pp: true }) : (currentPos)  }}<div class='map-mouse-position-toggle' ng-click='clickToggleDisplayType()'><i class='fa fa-exchange-alt'></i></div></div>",
            scope: {},
            link: function (scope, element, attrs, ctrl) {
                var olScope = ctrl.getOpenlayersScope();
